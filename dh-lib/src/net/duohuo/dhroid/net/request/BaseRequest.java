...
import net.duohuo.dhroid.net.adapter.CacheCall;
import net.duohuo.dhroid.net.adapter.Call;
import net.duohuo.dhroid.net.adapter.CallAdapter;
import net.duohuo.dhroid.net.adapter.DefaultCallAdapter;
...
import net.duohuo.dhroid.net.convert.Converter;
import net.duohuo.dhroid.net.https.HttpsUtils;
import net.duohuo.dhroid.net.model.HttpHeaders;
import net.duohuo.dhroid.net.model.HttpParams;
import net.duohuo.dhroid.net.request.ProgressRequestBody;
...
/**
 * ================================================ 作 者：廖子尧 版 本：1.0
 * 创建日期：2016/1/12 描 述：所有请求的基类，其中泛型 R 主要用于属性设置方法后，返回对应的子类型，以便于实现链式调用 修订历史：
 * ================================================
 */
public abstract class BaseRequest<R extends BaseRequest> {
	...
	protected List<Cookie> userCookies = new ArrayList<Cookie>(); // 用户手动添加的Cookie
	...
	public BaseRequest(String url) {
		...
		if (go.getCommonParams() != null)
			params.put(go.getCommonParams());
		if (go.getCommonHeaders() != null)
			headers.put(go.getCommonHeaders());
		// 添加缓存模式
	...
	@SuppressWarnings("unchecked")
	public R setCertificates(InputStream bksFile, String password,
			InputStream... certificates) {
		sslParams = HttpsUtils.getSslSocketFactory(bksFile, password,
				certificates);
		return (R) this;
	}
	...
	public R addCookie(String name, String value) {
		...
		Cookie cookie = builder.name(name).value(value).domain(httpUrl.host())
				.build();
	...
	/**
	 * 返回当前的请求方法 GET,POST,HEAD,PUT,DELETE,OPTIONS
	 */
		...
		progressRequestBody.setListener(new ProgressRequestBody.Listener() {
			...
			public void onRequestProgress(final long bytesWritten,
				...
				OkGo.getInstance().getDelivery().post(new Runnable() {
					...
					public void run() {
						if (mCallback != null)
							mCallback.upProgress(bytesWritten, contentLength,
	...
	public okhttp3.Call generateCall(Request request) {
		...
		if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0
				&& sslParams == null && userCookies.size() == 0) {
			return OkGo.getInstance().getOkHttpClient().newCall(request);
		} else {
			OkHttpClient.Builder newClientBuilder = OkGo.getInstance()
			...
			if (userCookies.size() > 0)
				OkGo.getInstance().getCookieJar().addCookies(userCookies);
